@if (isset($attribute))
    ...
    <v-lookup-component :attribute="{{ json_encode($attribute) }}" :validations="'{{ $validations }}'"
        :value="{{ json_encode($lookUpEntityData)}}" can-add-new="{{ $canAddNew ?? false }}"
        @lookup-added="handleLookupAdded" @lookup-removed="handleLookupRemoved">
...
@pushOnce('scripts')
    <script type="text/x-template" id="v-lookup-component-template">
                <div
                    class="relative"
                    ref="lookup"
                >
        ...
        app.component('v-lookup-component', {
            ...
            methods: {
                ...
                handleResult(result) {
                    ...
                    this.$emit('lookup-added-with-attribute', {
                        item: this.selectedItem,
                        attribute: this.attribute
                    });

                    // Dispatch DOM event for bubbling
                    this.$el.dispatchEvent(new CustomEvent('lookup-added-event', {
                        bubbles: true,
                        detail: {
                            item: this.selectedItem,
                            attribute: this.attribute
